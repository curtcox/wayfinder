"""wayfinder-exec-web CLI entry point (Phase 7b §9.10 extension)."""
...
from wayfinder.exec.web_loop import WebExecutorLoop


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="wayfinder-exec-web",
        description=(
            "Executor with browser/Playwright support (§9.10 extension beyond strict v0.1 policy)"
        ),
    )
    ...
    parser.add_argument(
        "--executor-id",
        default=os.environ.get("WAYFINDER_EXECUTOR_ID", "wayfinder-exec-web-local"),
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    command_name = f"wayfinder-exec-web.{args.command}"
    ...
    try:
        ...
        outcome = WebExecutorLoop(config, secrets_path=secrets_path).run()
        result = {
