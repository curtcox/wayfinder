"""wayfinder-exec CLI entry point (Phase 4)."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="wayfinder-exec")
    parser.add_argument(
        "--store",
    ...
    parser.add_argument(
        "--executor-id",
        default=os.environ.get("WAYFINDER_EXECUTOR_ID", "wayfinder-exec-local"),
        help="Executor actor id recorded in updates",
    )
    parser.add_argument(
        "--policy",
        type=Path,
        help="Policy file override (JSON or YAML)",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    command_name = f"wayfinder-exec.{args.command}"
